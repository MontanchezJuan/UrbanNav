...
export default class LicensesController {
  public async store({ request, response }: HttpContextContract) {
    try {
      ...
      return response
        .status(201)
        .json({ message: 'Licensia creada exitosamente', data: theLicense })
    } catch (error) {
      console.error(error)
      return response
        .status(500)
        .json({ message: 'Error al crear la Licensia', data: error.message })
    }
  }
  ...
  public async update({ params, request, response }: HttpContextContract) {
    try {
      ...
      return response
        .status(200)
        .json({ message: 'Licensia actualizada exitosamente', data: license })
    } catch (error) {
      console.error(error)
      return response
        .status(500)
        .json({ message: 'Error al actualizar la licensia', data: error.message })
    }
  }
